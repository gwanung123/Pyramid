...
define(["./models/model.kendo.grid", "kendo.all.min"],
   function( model ) {
      ...
      KendoGrid.prototype.setEvents = function( domID, options ) {
         ...
         self.setData = function( data, param ) {
            var self = this;
            var finalData = data || [],
                isChange = false;
            ...
            if ( options.setData !== undefined ) options.setData(self.TAG, finalData, self, domID);
            if ( nexus.setData !== undefined ) {
               var len = nexus.setData.length;
               for ( var i=0; i<len; i++ ) {
                  var cb = nexus.setData[i];
                  cb.callback(self.TAG, finalData, self, cb.callbackParam);
               }
            }
         };
