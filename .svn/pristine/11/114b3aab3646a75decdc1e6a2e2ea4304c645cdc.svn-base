...
define(["./models/model.kendo.grid", "kendo.all.min"],
   function( model ) {
      ...
      function KendoGrid( tag, domID, options ) {
         ...
         this.param.detailInit = this.onDetailInit;
      ...
      KendoGrid.prototype.setEvents = function( domID, options ) {
         ...
         self.onDetailInit = function( e ) {
            console.log("[KendoGrid] :: Grid : onDetailInit");
            if ( options.onDetailInit !== undefined ) options.onDetailInit(self.TAG, e, self, domID);
         };
