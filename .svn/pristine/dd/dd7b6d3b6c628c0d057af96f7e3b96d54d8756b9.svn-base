   ...
   function( _DBMS ) {
      ...
      Grid.prototype.getData = function( arrData ) {
         ...
         for ( var idx=0; idx<dataLen; idx++ ) {
            ...
            if ( data["ROUTED_COUNT"] === 0 ) {
               data["RATE"] = 0;
            }
            else {
               data["RATE"] = data["COMPLETE_COUNT"] / data["ROUTED_COUNT"];
            }
      ...
      Grid.prototype.makeTenantData = function( field, text ) {
         var self = this;
      ...
      Grid.prototype.getOptions = function( text, dbms, shared ) {
         ...
         options.toolbar = ["excel"];
         options.excel = {
            allPages: true,
            fileName: text.FILENAME + ".xlsx"
         };
         options.detailTemplate = kendo.template(
            "<table id=\"nx-campaign-container\" style=\"height: 30px;\">" +
               "<tbody>" +
               "<tr>" +
                  "<td>" +
                     "<div>" +
                        "<b class=\"nx-campaign-title\">Campaign Description</b>" +
                        "<div id=\"nx-campaign-desc\">" +
                        "#: MEMO #" +
                        "</div>" +
                        "<textarea style=\"display: none;\" id=\"nx-campaign-editor\" rows=\"10\" cols=\"100\" data-bind=\"value:MEMO\">" +
                        "#: MEMO #" +
                        "</textarea>" +
                     "</div>" +
                  "</td>" +
                  "<td>" +
                     "<div>" +
                        "<div id=\"nx-campaign-create\">CREATE : #: REG_ID # (#: REG_DATE #)</div>" +
                        "<div id=\"nx-campaign-update\">UPDATE : #: MOD_ID # (#: MOD_DATE #)</div>" +
                     "</div>" +
                  "</td>" +
               "</tr>" +
               "</tbody>" +
