...
define(['sammy', "nxbinder", "nxdefine", "Defines/text.define", "Components/components"],
   function( _SAMMY, _BINDER, _DEFINE, _TEXT, _Components ) {
      ...
      var changeURL = function( tag ) {
         ...
         if ( change === undefined ) {
            console.log("[master.router] :: sammy : changeURL = "+tag);
            return;
         }
      ...
      var _init = {
         pusherCallback: function( pushed, param ) {
            ...
            var pushCallbacks = _nxWinBlock.get("pushCallbacks", "shared"),
                callbacks = [],
                len = 0;
         ...
         resource: function() {
            ...
            options.position = { top: "56px", left: "70px" };
            options.title = false;
            options.animation = {
               open: {
         ...
         container: function() {
            ...
            options.draggable = false;
            options.resizable = false;
            options.position = { top: "56px", left: "370px" };
            options.title = false;
            options.container = "nx-contents";
            options.content = _DEFINE.getComponent(root, "content");
            options._nexus = _nxWinBlock;
      ...
      var application = _SAMMY(function() {
         ...
         this.get("master.html", function(context) {
            console.log("[master.router] :: sammy : application = "+application.getLocation());
