...
define([
   ...
   function( ko, _binder, _TEXT, _components ) {
      ...
      var DOMID = "nx-contents-agent";

      function agentViewModel( win ) {

         console.log("[agent] :: agentViewModel : load");
         var self = this;
         ...
         require(["nxdefine"], function( define ) {
            console.log("[agent] :: agentViewModel : define load");
            self._DEFINE = define;
            self._ROOT = self._DEFINE.MENU.AGENT.TAG;
         ...
         this._setTemplates = {
            ...
            init : function( view ) {
               ...
               that._viewLeft.init(that.nxWin.set({
                  text : _TEXT.CONTENTS.AGENT.LEFT
               }, "viewLeft"));

               that._viewRight.init(that.nxWin.set({
                  text : _TEXT.CONTENTS.AGENT.RIGHT
               }, "viewRight"));

               that._viewCenter.init(that.nxWin.set({
                  text : _TEXT.CONTENTS.AGENT.CENTER
               }, "viewCenter"));
            },
            _viewLeft : {
               handle : undefined,
      ...
      // prototype
      //===============================================================================//
      var _ptt = agentViewModel.prototype;

      _ptt.onContentLoad = function( view ) {
         console.log("[agent] :: agentViewModel : onContentLoad");
         var self = this;
