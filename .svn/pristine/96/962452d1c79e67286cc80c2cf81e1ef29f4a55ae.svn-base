      ...
      ViewLeftViewModel.prototype._setEvent = function( tag, options, components ) {
         ...
         self._meEvent = {
            setData : function( tag, data, methods, param ) {
               ...
               switch ( tag ) {
                  case "TREEVIEW_AGENT":
                     self._nxBlock.set({ _master: asis }, "shared");

                     var tenant = components["TREEVIEW_TENANT"];
                     tenant.model.requestData(tenant.option, {
                        onResult : function(result, error, param) {
                           if ( error !== null ) return;
                           if ( result.hasOwnProperty("result") ) return;
                           tenant.viewModel.setData(result, param);
                        }
                     });

                     var group = components["TREEVIEW_GROUP"];
                     group.model.requestData(group.option, {
                        onResult : function(result, error, param) {
                           if ( error !== null ) return;
                           if ( result.hasOwnProperty("result") ) return;
                           group.viewModel.setData(result, param);
                        }
                     });

                     var team = components["TREEVIEW_TEAM"];
                     team.model.requestData(team.option, {
                        onResult : function(result, error, param) {
                           if ( error !== null ) return;
                           if ( result.hasOwnProperty("result") ) return;
                           team.viewModel.setData(result, param);
                        }
                     });

                     var dn = components["TREEVIEW_DN"];
                     dn.model.requestData(dn.option, {
                        onResult : function(result, error, param) {
                           if ( error !== null ) return;
                           if ( result.hasOwnProperty("result") ) return;
                           dn.viewModel.setData(result, param);
                        }
                     });

                     var scenario = components["TREEVIEW_SCENARIO"];
                     scenario.model.requestData(scenario.option, {
                        onResult : function(result, error, param) {
                           if ( error !== null ) return;
                           if ( result.hasOwnProperty("result") ) return;
                           scenario.viewModel.setData(result, param);
                        }
                     });
                     break;

                  case "TREEVIEW_DN":
                     self._nxBlock.set({ _dn: asis }, "shared");

                     var route = components["TREEVIEW_ROUTE"];
                     route.model.requestData(route.option, {
                        onResult : function(result, error, param) {
                           if ( error !== null ) return;
                           if ( result.hasOwnProperty("result") ) return;
                           route.viewModel.setData(result, param);
                        }
                     });
                     break;

                  case "TREEVIEW_ROUTE":
                     self._nxBlock.set({ _route: asis }, "shared");
                     break;

                  case "TREEVIEW_SCENARIO":
                     self._nxBlock.set({ _scenario: asis }, "shared");
                     break;
               }
            },
            onSelect : function( tag, node, methods, param ) {
               var level = methods.getLevel(node),
                   selected = methods.getText(node);
               console.log("## ME ## ["+tag+"] onSelect: ("+level+") "+selected);
            }
         };

         switch ( tag ) {
            case "TREEVIEW_TENANT":
            case "TREEVIEW_GROUP":
            case "TREEVIEW_TEAM":
            ...
            case "TREEVIEW_ROUTE":
               ...
               //options.setPushData = self._meEvent.setPushData;
               options.onSelect = self._meEvent.onSelect;
               break;
