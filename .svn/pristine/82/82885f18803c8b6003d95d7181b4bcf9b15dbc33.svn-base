...
define(["knockout"],
   function( ko ) {
      ...
      Binder.prototype.bind = function( component, param ) {
         ...
         if ( !domID ) {
            return console.log("[interaction.binder] :: Binder : bind, check domID");
         }
         ...
         if ( typeof vm === "function" ) {
            ...
            if ( !Array.isArray(domID) ) {
               ...
               if ( $("#"+domID)[0] === undefined ) {
                  return console.log("[interaction.binder] :: Binder : bind, not find DOM");
               }
      ...
      Binder.prototype.appendHTML = function( parentID, HTML ) {
         /* view(HTML) append  -jquery- */
         this.emptyHTML(parentID);
