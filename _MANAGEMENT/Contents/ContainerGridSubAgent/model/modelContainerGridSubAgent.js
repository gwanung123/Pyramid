   ...
   function( _DBMS ) {
      ...
      Grid.prototype.makeDropDownList = function( field, params, shared ) {
         ...
         if ( arrPath[3] !== undefined ) {
            var shmemTmp = shmem[arrPath[3]];

            switch ( field ) {
               case "EMPLOYEE_ID":
                  for ( var i=0; i<shmemTmp.length; i++ ) {
                     var agent = shmemTmp[i];
                     if ( agent["MEDIA_ID"] < 10 ) continue;
                     data.push(agent);
                  }
                  break;

               case "MASTER_EMPLOYEE_ID":
                  for ( var i=0; i<shmemTmp.length; i++ ) {
                     var agent = shmemTmp[i];
                     if ( agent["MEDIA_ID"] >= 10 ) continue;
                     data.push(agent);
               ...
               case "SUB_DN":
                  for ( var i=0; i<shmemTmp.length; i++ ) {
                     var dn = shmemTmp[i];
                     if ( dn["MEDIA_ID"] < 10 ) continue;
                     data.push(dn);
                  }
                  break;

               default:
                  data = $.extend(true, [], shmemTmp);
            }
         }
         else {
            switch ( params.value ) {
               case "MEDIA_ID":
                  for ( var i=0; i<shmem.length; i++ ) {
                     var media = shmem[i];
                     if ( media["MEDIA_KIND"] < 10 ) continue;
                     data.push(media);
                  }
                  break;
      ...
      Grid.prototype.setColumns = function( action, text, options, _shared ) {
         ...
         options.nexus.group = {
            ...
            aggregates: [
               { field: "MASTER_EMPLOYEE_ID", aggregate: "count" }
            ]
         };
