...
define(["../../../Library/master.dbms"],
   function( _DBMS ) {
      ...
      Grid.prototype.setItems = function( arrData, action, options ) {
         var self = this;
         ...
         for ( var i=0; i<len; i++ ) {
            ...
            for ( var field in argsFields ) {
               arr.push(self.getValue(data.source[field], argsFields[field]));
            }
      ...
      Grid.prototype.setColumns = function( fromTag, action, text, options, _shared ) {
         var self = this;
         var txt = text[fromTag],
             dbms = options.nexus.dbms[txt.DATA];
         var columns = $.extend(true, [], txt.COLUMN),
             schema = txt.SCHEMA.model.fields,
             pk = [],
             sort = [],
             fields = [];
      ...
      Grid.prototype.getOptions = function( text, dbms ) {
         var self = this;
