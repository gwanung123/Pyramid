            ...
            setData : function( tag, data, methods, param ) {
               ...
               switch ( tag ) {
                  case "TREEVIEW_AGENT":
                     ...
                     var skill = components["TREEVIEW_SKILL"];
                     skill.model.requestData(skill.option, {
                        onResult : function(result, error, param) {
                           if ( error !== null ) return;
                           if ( result.hasOwnProperty("result") ) return;
                           skill.viewModel.setData(result, param);
                        }
                     });
                  ...
                  case "TREEVIEW_SKILL":
                     self._nxBlock.set({ _skill: asis }, "shared");
                     break;

                  case "TREEVIEW_DN":
                     self._nxBlock.set({ _dn: asis }, "shared");

                     var queue = components["TREEVIEW_QUEUE"];
                     queue.model.requestData(queue.option, {
                        onResult : function(result, error, param) {
                           if ( error !== null ) return;
                           if ( result.hasOwnProperty("result") ) return;
                           queue.viewModel.setData(result, param);
                        }
                     });
                     break;

                  case "TREEVIEW_QUEUE":
                     self._nxBlock.set({ _queue: asis }, "shared");
                     break;
      ...
      ViewLeftViewModel.prototype._setJoinEvent = function( tag, join, component ) {
         ...
         self._joinEvent = {
            setPushData : function( tag, action, service, id, data, bind, methods, param ) {
               ...
               if ( component.model.tag === "TREEVIEW_QUEUE" ) {
                  var asis = component.viewModel.getData("AS_IS"),
                      pushAction = component.model.getPushData(action, asis, data);
                  if ( pushAction === null ) return;
                  component.viewModel.setPushData(pushAction, service, id, data, bind);
               }
               else {
                  component.viewModel.setPushData(action, service, id, data, bind);
               }
