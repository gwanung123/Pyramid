...
define(['sammy', "nxbinder", "nxdefine", "Defines/text.define", "Components/components"],
   function( _SAMMY, _BINDER, _DEFINE, _TEXT, _Components ) {
      ...
      var changeURL = function( tag ) {
         ...
         if ( change === undefined ) {
            console.log("[monitor.router] :: sammy : changeURL = "+tag);
            return;
         }
      ...
      var _init = {
         pusherCallback: function( pushed, param ) {
            ...
            for ( var i=0; i<len; i++ ) {
               ...
               if ( callback.tag !== pushed.tag ) continue;
         ...
         board: function() {
            var root = _DEFINE.MENU.BOARD.TAG,
                parent = _DEFINE.getComponent(root, "parent"),
                windowID = "nx-contents-div-"+root;
            var windowVM = { options: { _nexus: _nxWinBlock } };

            _BINDER.appendHTML(parent, _DEFINE.getComponent(root, "view"));
            _BINDER.bind(_DEFINE.getComponent(root, "viewModel"), {winID: windowID, winVM: windowVM});
      ...
      var application = _SAMMY(function() {
         ...
         this.get("monitor.html", function(context) {
            console.log("[monitor.router] :: sammy : application = "+application.getLocation());
